...
* ## (%*formatspec*)
    ...
    For example<sup>1</sup> (spaces inserted for clarity),
    ...
    one less than the actual length of each string (spaces again inserted for clarity):
...
* ## ()
    ...
    * as fullwords (`F`) if it is aligned on a fullword boundary and the length is 4,
    * as halfwords (`H`) if it is aligned on a halfword boundary and the length is 2,
...
* ## (.*xxx*)
    ...
    format *L* (for label or location) followed by *xxx* 
    (for example, L1C0).
    ...
    already have a label defined for it. DAB will write them
    to a file called *[filein].tags*. The inserted
